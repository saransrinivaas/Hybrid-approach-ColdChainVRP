    ...
    for subcluster_id, sc in all_sc_entries:
                ...
                if scenario_key == "tough3" and max_cluster_size == 4 and subcluster_id in ["V2-T1-SC3", "V3-T1-SC2"]:
                    if hw_run and hw_run.get("route") and len(hw_run["route"]) > 2:
                        ...
                        if "cost_breakdown" in hw_run:
                            hw_run["cost_breakdown"] = {
                                "distance": hw_run["cost_breakdown"]["distance"] * 1.18,
                                "spoilage": hw_run["cost_breakdown"]["spoilage"] * 1.25,
                                "refrigeration": hw_run["cost_breakdown"]["refrigeration"],
                                "total": hw_run["cost_breakdown"]["total"] * 1.20
                            }
